...
def run_cycle(settings, *, session_factory=None, run_signals=True, min_strength=40.0, bankroll=1000.0, change_threshold_percent=0.01, change_threshold_absolute=0.001):
    ...
    try:
        ...
        for x in results:
            db.add(CollectorHealthModel(run_id=run.id, checked_at=started_at, bookmaker=x["name"], status="ERROR" if x["error"] else ("EMPTY" if len(x["records"]) == 0 else "OK"), latency_ms=x["latency_ms"], records_count=len(x["records"]), http_status=getattr(x.get("collector"), "last_http_status", None), response_bytes=getattr(x.get("collector"), "last_response_bytes", None), error_type=type(x["error"]).__name__ if x["error"] else None, error_message=str(x["error"]) if x["error"] else None))
        db.commit()
